...
use rustfs_disk_core::{DiskError as Error, Result};
...
mod test {
    ...
    fn test_endpoint_grid_host() {
        ...
        let endpoint = Endpoint::try_from("https://example.com/path").unwrap();
        assert_eq!(endpoint.grid_host(), "https://example.com");
    ...
    fn test_endpoint_clone_and_equality() {
        ...
        let endpoint3 = Endpoint::try_from("http://example.com:9000/path").unwrap();
    ...
    fn test_endpoint_with_special_paths() {
        ...
        #[cfg(windows)]
        {
            let endpoint = Endpoint::try_from("C:\\path\\to\\data").unwrap();
    ...
    fn test_url_parse_from_file_path() {
        ...
        let result = url_parse_from_file_path("192.168.1.1/path");
    ...
    fn test_endpoint_hash() {
        ...
        let endpoint1 = Endpoint::try_from("/tmp/data").unwrap();
        let endpoint2 = Endpoint::try_from("/tmp/data").unwrap();

        let mut map = HashMap::new();
        map.insert(endpoint1, "value1");
        assert_eq!(map.get(&endpoint2), Some(&"value1"));
    }
}
